...
* EventBinding
    ...
    ```
    語法上 Event Binding 為 `(click)="onCreateServer()"`，使用 `(..)` 來代表 html Element 所想要綁定的事件，等號的右邊則放入在 TypeScript 中寫好的 function，不過也可以直接在等號右邊放入 TypeScript 述句，但通常不會這麼做，利用 function 將要做的事情封裝起來可以讓 template 的版面部會出現太多邏輯的部分。
