...
一樣在 template 中加上範本參考變數，然後在 TypeScript 的變數前加上 `@ViewClild`，小括號內部要放入 `serverContentInput`( 範本參考變數 ) 以及 `{static: true}`，這樣就可以直接觀察到 template 中的元素，而除了指定 html Element 外，其實也可以指定自己定義的 Component ( 想像成自己定義的 html Element 一樣 )。
...
我們刻意把原本存在於 `server-element.component.html` 的部分 template 移出，放到 `app.component.html` 中，並且把它放到 `<app-server-element>` 這個自行定義的 Element 之間，然後在子元素使用 `<ng-content>` 取代原本元素的位置，這樣就可以完成在父元素自行定義部份 template 的部分。這時注意到的話，應該會發現 `<p>` 變得可以吃到藍色的字體，原因在於我們將其定義在外部的 Component 了。

### 生命週期

![  ](images/8-4.png)

這張圖是從 Udemy 截下來的圖片，在下面我們會用程式展現這些 Angular Component 的生命週期，首先先以文字來解釋這個生命週期的問題 :

* ngOnChanges : 當綁定 `@Input` 的屬性有所異動時觸發。
* ngOnInit : 當元件被初始化時觸發。
* ngDoCheck : 
...
最後則是使用 `@ContentChild()`，使用方式跟 `@ViewChild` 其實一樣，只是一個是鎖定本身 Component 的 template，另一個則是鎖定外部 Component 傳進來的 Element :
