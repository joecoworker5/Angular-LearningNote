...
這是一件好事情，如果純粹用 JavaScript、html、css 三個文檔所組成的網頁，很常會有擔心 style 互相影響的問題，Angular 讓所有 Component 產生出來的元素都會有特有的屬性，讓它們的 style 不會有交互影響。這樣的設定是可變動的，只要在 `@Component()` 上增加新的屬性 :
...
除了利用事件傳入對於 DOM 元素的參考，另一種做法是 `@ViewChild()`，它可在 TypeScript 中直接指到 Template 的元素而不必依靠事件來傳入 :
