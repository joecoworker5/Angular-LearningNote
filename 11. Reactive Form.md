...
在 TypeScript 可以在 `FormGroup` 中包入另一個 `FormGroup`，並且給予這個 `FormGroup` 另一個名稱，而 template 就必須在額外添加一個 `<div>`，並給予它 `formGroupName` 來代表內部 `FormGroup` 的 key 值。如果有加上 `FormGroup`，後面的 `FormControl` 要有 `.` 的語法來取用。
...
先看到 TypeScript 的部分，在最原本的 `FormGroup` 中新增一個 key 值並對應到 `FormArray` 物件，我們希望在前端按下按鈕後，會新增一個 `<input>`，因此在這邊新增一個 `onAddHobby(..)` 方法，而 `get hobbies(..)` 則是為了讓前端 template 的部分可以使用 `*ngFor` 而開出的 `get(..)` 方法。template 中則多出一整個 `<div>` 區塊，並給予 `formArrayName` 名稱對應到 TypeScript 的部分，另外使用 `*ngFor` 來把 TypeScript 中的 `FormArray` 給一一迭代出來。
...
新增了一個驗證 email 的方法，傳入參數一樣定義為 `FormControl`，回傳則定義非同步物件 `Promise` 或是 `Observable`，並且寫下方法內部的細節，這邊所採用的是 `Promise`，並用 `setTimeout` 假設為非同步的驗證，驗證通過就 `resolve(..)`，驗證失敗則在內部傳入物件，並且最後必須回傳這個 `Promise` 物件。如果打開開發人員工具，應該會發現當 `<input>` 內部有所變動時，原本的 `ng-valid`、`ng-invalid` 應該會轉變為 `ng-pending` 一下子，原因就在於 `setTimeout(..)` 的非同步設定。
