...
`*ngFor` 的使用方式是在想迭代的元素增加 `*ngFor="let server of servers"`，`servers` 在這邊是 TypeScript 中的陣列，`server` 就代表每次迭代的元素。只要在 TypeScript 中增加新元素到 `servers` ( 也就是觸發 `onCreateServer()` )，`*ngFor` 得迭代也會直接反應到畫面上。
...
另一個更改元素的做法，是在 `constructor(..)` 代入另一個物件 `Renderer2`，使用指令 `ng g d better-highlight --spec false` 產生元件，再更改成以下內容 :
...
`@HostListener()` 則是另一個可能在 Directive 中用到 Decorator，會利用它來監聽事件，來把程式碼改成以下的形式 :
