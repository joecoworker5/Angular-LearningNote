...
```
在 Template 主要多了 `<p *ngIf="serverCreated">Server was created, server name is {{serverName}}</p>`，`*ngIf` 前面帶有星號，代表它是個結構型指令，而右邊則放 boolean 值，決定是否要呈現此元素。TypeScript 的部分則在 `onCreateServer()` 中，改變 `serverCreated` 的狀態，這樣按下按鍵後會呈現出 `<p>` 元素。
...
除了內建的 Directive，還可自行定義 Directive 放在各個元素上面，先用指令 `ng g d basic-highlight --spec false` ( `--spec false` 是比較舊的語法，新的 CLI 要再查 Angular 官網 ) 產生出 Directive 檔，並改成以下的形式 :
